...
| **Destructive actions go through proposals.** Under `AGENT_TAINT_ROUTING`, an outside client's risky writes currently wait for a person. Opening a proposal for every destructive call is slice S5 (#798, in review). | `Modules/Mcp/taintHold.js` | `tests/mcp-oauth-actions.test.js` "holds a risky write from an OAuth client for a person, and leaves an audited refusal". A proposal test ships with #798. | |
