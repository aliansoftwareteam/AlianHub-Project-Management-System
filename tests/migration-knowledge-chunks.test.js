...
const { knowledgeChunksSchema, knowledgeIndexStateSchema, knowledgeExclusionsSchema } = require('../utils/mongo-handler/createSchema');
...
const EXCLUSIONS = SCHEMA_TYPE.KNOWLEDGE_EXCLUSIONS;
...
const EXCLUSION_INDEX = { name: 'kind_1_sourceType_1_sourceId_1_userId_1', key: { kind: 1, sourceType: 1, sourceId: 1, userId: 1 }, unique: true };
...
const tenants = ({ broken = [] } = {}) => {
    ...
    const crud = async (companyId, q, method) => {
        ...
        if (method === 'createIndexes') {
            ...
            indexes[`${companyId}:${q.type}`] = { [CHUNKS]: [ID_INDEX, TEXT_INDEX, SOURCE_INDEX], [STATE]: [ID_INDEX, STATE_INDEX], [EXCLUSIONS]: [ID_INDEX, EXCLUSION_INDEX] }[q.type];
            return undefined;
        }
...
const RECORDED = { textIndex: 'text_text', sourceIndex: 'sourceType_1_sourceId_1_ordinal_1', stateIndex: 'sourceType_1', exclusionIndex: 'kind_1_sourceType_1_sourceId_1_userId_1' };
...
describe('025-knowledge-chunks', () => {
    ...
    it('the chunk schema declares a text index over the chunk text and one chunk per source and ordinal', () => {
        ...
        expect(knowledgeExclusionsSchema.indexes()).toContainEqual([{ kind: 1, sourceType: 1, sourceId: 1, userId: 1 }, expect.objectContaining({ unique: true })]);
    });
    ...
    it('declares every field a chunk is written with, since the schema is strict', () => {
        ...
        ['companyId', 'kind', 'sourceType', 'sourceId', 'userId', 'reason']
            .forEach((field) => expect(knowledgeExclusionsSchema.path(field)).toBeDefined());
        expect(knowledgeExclusionsSchema.get('strict')).toBe(true);
    });

    it('builds every knowledge collection\'s indexes on every tenant without dropping any, and records what it found', async () => {
        const db = tenants();
        ...
        expect(db.methods.every(([, type]) => [CHUNKS, STATE, EXCLUSIONS].includes(type))).toBe(true);
        ...
        expect(db.methods.filter(([, , method]) => method === 'createIndexes').map(([companyId, type]) => `${companyId}:${type}`))
            .toEqual([`c1:${CHUNKS}`, `c1:${STATE}`, `c1:${EXCLUSIONS}`, `c2:${CHUNKS}`, `c2:${STATE}`, `c2:${EXCLUSIONS}`]);
