...
describe('BOARD EMPTY - three states, never No Data Found', () => {
    ...
    test('loading, honest empty, and failed bind are distinct', () => {
        ...
        expect(countPaintedTaskRows([
            { tasksArray: [] },
            { tasksArray: [] },
            { tasksArray: [] },
        ])).toBe(0);
        expect(countPaintedTaskRows([
            { tasksArray: [{ _id: 'ghost' }] },
            { tasksArray: [] },
            { tasksArray: [] },
        ])).toBe(0);
        expect(boardHoursVisible(sprintSurfaceKind({
            paintedCount: countPaintedTaskRows([{ tasksArray: [] }, { tasksArray: [] }, { tasksArray: [] }]),
            sidebarCount: 7,
        }))).toBe(false);
        expect(paintSprintGroups(
        ...
        expect(dropCss).toContain('.dropdown-back-drop');
        expect(dropCss).toContain('pointer-events: none');
        const searchOpen = fs.readFileSync(path.join(__dirname, '..', 'frontend', 'src', 'utils', 'openGlobalSearch.js'), 'utf8');
        expect(searchOpen).toContain('lastSearchTasks');
        expect(searchOpen).toContain('rememberSearchTasks');
