...
jest.mock('../utils/mongo-handler/mongoQueries', () => ({ MongoDbCrudOpration: (...a) => crud(...a) }));
...
describe('markUndone', () => {
    it('throws when the unchained mark write fails instead of swallowing it', async () => {
        crud.mockRejectedValueOnce(new Error('db down'));
        await expect(audit.markUndone(C, ROW, 'u1')).rejects.toThrow('db down');
    });

    it('resolves once the mark lands', async () => {
        crud.mockResolvedValueOnce({ matchedCount: 1, modifiedCount: 1 });
