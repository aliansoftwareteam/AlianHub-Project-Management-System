...
const migration = require('../migrations/027-knowledge-embeddings');
...
const BUILT = { [CHUNKS]: [ID_INDEX, { name: 'sourceType_1_embeddingModel_1_sourceUpdatedAt_-1', key: KEY }] };
const RECORDED = { embeddingIndex: 'sourceType_1_embeddingModel_1_sourceUpdatedAt_-1' };
...
describe('027-knowledge-embeddings', () => {
    it('is a valid company-scoped migration listed after 026', () => {
        expect(() => validateMigration(migration, '027-knowledge-embeddings')).not.toThrow();
        expect(migration.scope).toBe('company');
        const ids = listMigrations().map((m) => m.id);
        expect(ids.indexOf('027-knowledge-embeddings')).toBe(ids.indexOf('026-knowledge-sources') + 1);
    });

    it('declares the vector on the strict chunk schema as an array of numbers, empty by default, beside the model version', () => {
        const embedding = knowledgeChunksSchema.path('embedding');
        expect(embedding).toBeDefined();
        expect(embedding.instance).toBe('Array');
        expect(embedding.caster.instance).toBe('Number');
        expect(embedding.defaultValue()).toEqual([]);
        expect(knowledgeChunksSchema.path('embeddingModel')).toBeDefined();
        expect(knowledgeChunksSchema.get('strict')).toBe(true);
        expect(knowledgeChunksSchema.indexes()).toContainEqual([KEY, expect.any(Object)]);
    });

    it('builds the candidate index on every tenant with createIndexes, never syncIndexes, and records its name', async () => {
        const db = tenants();
        ...
        expect(logger.info).toHaveBeenCalledWith(expect.stringContaining('027 c1'));
    });
    ...
    it('fails a tenant whose index is still missing after the build', async () => {
        ...
        expect(ctx.companies.c1).toMatchObject({ ok: false, error: expect.stringContaining('knowledge_chunks embedding') });
    });
});
