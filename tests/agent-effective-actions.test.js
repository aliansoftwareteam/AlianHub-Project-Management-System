...
describe('GET /api/v2/agents/manifest', () => {
    it('lists every non-deleted agent with its skills, their source and effective actions', async () => {
        ...
        expect(r.body.data[0]).toEqual({
            ...
            skills: [
                { key: 'brief.parse', name: 'Intake', enabled: true, resolved: true, source: 'code', version: null, inputs: ['brief'], requires: { code: 'brief', needs: 'it needs a brief of at least 40 characters', scope: 'task' }, emits: ['subtask.create', 'task.comment'], risk: 'low', effectiveActions: ['task.comment'], unavailable: null },
                { key: 'task.summary', name: 'Summariser', enabled: true, resolved: true, source: 'data', version: 1, inputs: ['brief'], requires: { code: 'brief', needs: 'it needs a brief of at least 40 characters', scope: 'task' }, emits: ['task.comment'], risk: 'low', effectiveActions: ['task.comment'], unavailable: null },
                { key: 'gone.skill', name: 'gone.skill', enabled: false, resolved: false, source: null, version: null, inputs: [], requires: null, emits: [], risk: null, effectiveActions: [], unavailable: null },
            ],
        });
