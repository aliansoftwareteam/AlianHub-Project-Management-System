    ...
    <div class="ah-page ai-page">
        ...
        <div class="ai-page__main">
            <div class="ah-toolbar">
                <div class="ah-toolbar__title">{{ $t('AiV2.inbox') }}</div>
                <div class="ah-toolbar__spacer"></div>
                <span v-if="undo" class="ah-chip ah-chip--ok">
                    {{ $t('AiV2.approved') }}
                    <button type="button" class="ah-btn ah-btn--ghost ah-btn--sm" @click="onUndo">{{ $t('AiV2.undo') }}</button>
                </span>
            </div>
            ...
            <div class="ai-inbox" :class="{ 'ai-inbox--detail': selected }">
                <div class="ai-inbox__list ah-scroll">
                    ...
                    <div v-if="loading" class="ah-empty" style="margin:14px">{{ $t('AiV2.loading') }}</div>
                    <div v-else-if="!proposals.length" class="ai-done">
                        <div class="ai-done__n ah-mono">{{ counts.approved || 0 }}</div>
                        <p class="ah-h3">{{ $t('AiV2.queue_clear') }}</p>
                        <p class="ah-small">{{ $t('AiV2.queue_clear_body', { approved: counts.approved || 0, declined: counts.declined || 0 }) }}</p>
                    </div>
                    ...
                    >
                        <div class="ai-item__top">
                            ...
                            <span v-if="p.gate" class="ah-chip ah-chip--warn ah-chip--mono">{{ $t('AiV2.gated') }}</span>
                ...
                <div v-else class="ai-detail ah-scroll">
                    ...
                    <div v-for="(change, i) in editable" :key="i" class="ai-change">
                        ...
                        <span v-if="!change.reversible" class="ah-chip ah-chip--warn">{{ $t('AiV2.not_reversible') }}</span>
                        <button v-if="editing" type="button" class="ah-btn ah-btn--ghost ah-btn--sm" @click="editable.splice(i, 1)">{{ $t('AiV2.drop') }}</button>
                    </div>

                    <div v-if="selected.gate" class="auth__banner auth__banner--warn" style="margin-top:14px">
                        <ShellIcon name="shield" :size="15" />
                        <span>{{ $t('AiV2.gate_note') }}</span>
                    </div>

                    <div v-if="error" class="ah-field__error" style="margin-top:12px">{{ error }}</div>

                    <div class="ai-actions">
                        <button type="button" class="ah-btn ah-btn--primary" :disabled="busy || !editable.length" @click="onApprove">{{ $t('AiV2.approve') }}</button>
                        <button type="button" class="ah-btn ah-btn--secondary" :disabled="busy" @click="editing = !editing">
                            {{ editing ? $t('AiV2.done_editing') : $t('AiV2.edit_then_approve') }}
                        </button>
                        <button type="button" class="ah-btn ah-btn--ghost" :disabled="busy" @click="onDecline">{{ $t('AiV2.decline') }}</button>
                    </div>
...
const tabs = computed(() => [
    { key: "pending", label: "AiV2.waiting", count: counts.value.pending || 0 },
    { key: "done", label: "AiV2.done_by_ai", count: counts.value.approved || 0 },
    { key: "declined", label: "AiV2.declined", count: counts.value.declined || 0 }
]);

const changesLabel = computed(() => {
    const list = editable.value;
    const all = list.length && list.every((c) => c.reversible);
    const unit = list.length === 1 ? t("AiV2.action_one") : t("AiV2.action_other");
    return t("AiV2.what_changes", { n: list.length, unit, note: all ? t("AiV2.all_reversible") : t("AiV2.some_permanent") });
});

const costLine = computed(() => {
    const c = selected.value?.cost || {};
    if (!c.tokens && !c.usd) return t("AiV2.logged_to_audit");
    return t("AiV2.cost_line", { tokens: c.tokens || 0, usd: Number(c.usd || 0).toFixed(2) });
});
...
const onApprove = async () => {
    ...
    try {
        ...
        const id = await afterDecision(t("AiV2.applied"));
        if (out?.undoUntil) {
...
const onDecline = async () => {
    ...
    try {
        ...
        await afterDecision(t("AiV2.declined_done"));
    } catch (e) {
        error.value = e.message;
    } finally {
        busy.value = false;
...
const onUndo = async () => {
    ...
    try {
        ...
        $toast.success(t("AiV2.undone"), { position: "top-right" });
    } catch (e) {
