    ...
    <div class="ah-page ai-page">
        ...
        <div class="ai-page__main">
            <div class="ah-toolbar">
                <div class="ah-toolbar__title">{{ $t('AiV2.agents') }}</div>
                <div class="ah-toolbar__spacer"></div>
                <button type="button" class="ah-btn ah-btn--primary ah-btn--sm" @click="creating = true">
                    <ShellIcon name="plus" :size="14" />{{ $t('AiV2.new_agent') }}
                </button>
            </div>

            <div class="ai-page__body ah-scroll">
                <p class="ai-lead">{{ $t('AiV2.hub_lead') }}</p>

                <div v-if="lastError" class="ah-empty">{{ lastError }}</div>
                <div v-else-if="loading" class="ah-empty">{{ $t('AiV2.loading') }}</div>

                <div v-else-if="!agents.length" class="ah-card ai-agent">
                    <h3 class="ah-h3">{{ $t('AiV2.empty_title') }}</h3>
                    <p class="ai-lead" style="margin:6px 0 12px">{{ $t('AiV2.empty_body') }}</p>
                        ...
                        <div class="ai-agent__top">
                            ...
                            <div class="ai-agent__id">
                                <div class="ai-agent__name">
                                    ...
                                    <span v-if="agent.paused" class="ah-chip ah-chip--warn">{{ $t('AiV2.paused') }}</span>
                                </div>
                ...
                <section class="ah-card ai-agent ai-ladder">
                    <div class="ah-label">{{ $t('AiV2.ladder') }}</div>
                    ...
                    <p class="ai-ladder__rule">{{ $t('AiV2.ladder_rule') }}</p>
                    <p class="ai-ladder__rule"><strong>{{ $t('AiV2.never_label') }}</strong> <span class="ah-mono">{{ neverList }}</span></p>
                </section>
            </div>
        </div>
...
const autonomyChip = (agent) => {
    ...
    return `${a.key} · ${t(`AiV2.autonomy_${a.level}`).toUpperCase()}`;
};
const ladderLabel = (step) => t(`AiV2.autonomy_${step.level}`);

const scopeOf = (agent) => {
    const ids = agent.projectIds || [];
    return ids.length ? t("AiV2.scope_projects", { n: ids.length }) : t("AiV2.scope_all");
};

const triggerOf = (agent) => {
    const s = agent.schedule || {};
    if (s.cron) return s.cron;
    if (s.at) return t("AiV2.every_day_at", { at: s.at });
    if (s.on) return s.on;
    return t("AiV2.manual_only");
};

const todayLine = (agent) => {
    const row = (spend.value.agents || []).find((a) => a.agentId === String(agent._id));
    if (!row || !row.runs) return t("AiV2.no_runs_today");
    return t("AiV2.today_runs", { runs: row.runs, usd: Number(row.usd || 0).toFixed(2) });
};
...
const onRunNow = async (agent) => {
    ...
    try {
        ...
        $toast.success(t("AiV2.run_started", { name: agent.name }), { position: "top-right" });
    } catch (error) {
