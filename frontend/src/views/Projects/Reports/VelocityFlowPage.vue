<template>
    <div class="ah-page rp-page">
        <div class="rp-head">
            <h1 class="rp-title">{{ $t('Reports.velocity_title') }}</h1>
        ...
        <div class="rp-two rp-two--even">
            <div class="rp-card">
                ...
                <div v-if="rows.length" class="rp-bars">
                    <div v-for="(row, i) in rows" :key="row.sprintId" class="rp-bar">
                        <div class="rp-bar__pair">
                            <span
                                ...
                                :title="$t('Reports.completed_n', { n: completedOf(row) })"
                            ></span>
                            <span
                    ...
                    <div v-if="forecast.ok" class="rp-bar">
                        ...
                        <span class="rp-bar__label">{{ $t('Reports.next') }}</span>
                    </div>
                </div>
                <div v-else class="rp-empty">
                    <strong>{{ $t('Reports.velocity_empty_title') }}</strong>
                    <span>{{ $t('Reports.velocity_empty_body') }}</span>
                </div>
            ...
            <div class="rp-card">
                <div class="rp-card__head">
                    {{ $t('Reports.cfd') }}
                    <span class="rp-card__note">{{ $t('Reports.cfd_note') }}</span>
                </div>
                <ApexChart v-if="cfdDays.length" type="area" height="240" :options="cfdOptions" :series="cfdSeries" />
                <div v-else class="rp-empty"><span>{{ $t('Reports.cfd_empty') }}</span></div>
            </div>
        </div>
...
const CFD_BANDS = [
    ...
    { key: 'onhold', label: 'Reports.band_blocked', color: '#c1121f' },
    { key: 'open', label: 'Reports.band_todo', color: '#9aa0b4' },
];
