<template>
    <div class="ah-page rp-page">
        ...
        <div v-if="rows.length" class="rp-table">
            <div class="rp-thead">
                <span>{{ $t('ReportsV2.col_milestone') }}</span>
                <span>{{ $t('ReportsV2.col_baseline') }}</span>
                <span>{{ $t('ReportsV2.col_now') }}</span>
                <span>{{ $t('ReportsV2.col_slip') }}</span>
                <span>{{ $t('ReportsV2.col_state') }}</span>
            </div>
            <div v-for="row in rows" :key="row.milestoneId" class="rp-tr rp-tr--static" :class="{ 'is-missed': row.state === 'missed' }">
                ...
                <span class="ah-chip" :class="chipClass(row.state)">{{ $t(`ReportsV2.state_${row.state.replace('-', '_')}`) }}</span>
            </div>
        </div>

        <div v-else-if="!loading" class="rp-empty">
            <strong>{{ $t('ReportsV2.milestones_empty_title') }}</strong>
            <span>{{ $t('ReportsV2.milestones_empty_body') }}</span>
        </div>

        <p class="rp-note">{{ $t('ReportsV2.milestones_footnote') }}</p>
    </div>
</template>
...
const headline = computed(() => t('ReportsV2.milestones_head', {
...
const subLine = (row) => {
    const parts = [row.projectName || t('ReportsV2.untitled_project'), t('ReportsV2.n_tasks', { n: row.tasks })];
    if (row.lastMove && row.lastMove.by) {
        parts.push(t('ReportsV2.moved_by', { who: row.lastMove.by, when: fullDate(new Date(row.lastMove.at).getTime()) }));
    }
    return parts.join(' · ');
};

const slipText = (row) => {
    if (row.state === 'missed') return t('ReportsV2.missed');
...
const load = async () => {
    ...
    try {
        ...
        if (body && body.status) {
            ...
            error.value = (body && body.statusText) || t('ReportsV2.load_failed');
            rows.value = [];
        }
    } catch (e) {
        error.value = e?.message || t('ReportsV2.load_failed');
        rows.value = [];
    } finally {
        loading.value = false;
    }
