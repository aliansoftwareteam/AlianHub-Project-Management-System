<template>
    <div>
        <div class="in-actions">
            <div class="ah-tabs">
                <button v-for="k in kinds" :key="k" type="button" class="ah-tab" :class="{ 'is-active': kind === k }" @click="kind = k; load()">{{ $t(`Instance.log_${k}`) }}</button>
            </div>
            <select v-model.number="lines" class="ah-input in-lines" @change="load">
                <option v-for="n in [200, 500, 2000]" :key="n" :value="n">{{ $t('Instance.last_lines', { n }) }}</option>
            </select>
            <div class="ah-toolbar__spacer"></div>
            <button type="button" class="ah-btn ah-btn--secondary ah-btn--sm" :disabled="busy" @click="load">{{ $t('Instance.refresh') }}</button>
            <button type="button" class="ah-btn ah-btn--ghost ah-btn--sm" :disabled="busy || !tail.file" @click="download(`${env.INSTANCE_LOG_DOWNLOAD}?name=${tail.file}`, tail.file)"><ShellIcon name="download" :size="14" />{{ $t('Instance.download') }}</button>
        </div>
        <p class="ah-small">{{ tail.file ? $t('Instance.log_reading', { file: tail.file, size: formatBytes(tail.size) }) : $t('Instance.log_none') }} <a :href="guide('troubleshooting')" target="_blank" rel="noopener">{{ $t('Instance.guide') }}</a></p>
        <div v-if="error" class="in-banner in-banner--danger"><ShellIcon name="alert" :size="15" /><span>{{ error }}</span></div>
        <pre v-else class="in-pre">{{ tail.lines.length ? tail.lines.join('\n') : $t('Instance.log_empty') }}</pre>

        <section v-if="files.length" class="ah-card in-card">
            <div class="in-card__head"><span class="in-card__title">{{ $t('Instance.log_files') }}</span><span class="ah-small ah-mono">{{ dir }}</span></div>
            <table class="in-table">
                <thead><tr><th>{{ $t('Instance.file') }}</th><th>{{ $t('Instance.size') }}</th><th>{{ $t('Instance.modified') }}</th><th></th></tr></thead>
                <tbody>
