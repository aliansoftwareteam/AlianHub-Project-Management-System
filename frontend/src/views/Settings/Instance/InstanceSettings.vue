<template>
    <div>
        ...
        <div v-else-if="!rows.length" class="ah-empty">{{ $t('InstanceV2.loading') }}</div>
        <template v-else>
            <div class="in-actions">
                <div class="ah-tabs">
                    <button v-for="g in groups" :key="g" type="button" class="ah-tab" :class="{ 'is-active': group === g }" @click="group = g">{{ $t(`InstanceV2.group_${g}`) }}</button>
                </div>
                <div class="ah-toolbar__spacer"></div>
                <button v-if="testable" type="button" class="ah-btn ah-btn--secondary ah-btn--sm" :disabled="busy" @click="test">{{ $t('InstanceV2.test') }}</button>
                <button type="button" class="ah-btn ah-btn--primary ah-btn--sm" :disabled="busy || !dirty" @click="save">{{ busy ? $t('InstanceV2.saving') : $t('InstanceV2.save') }}</button>
            </div>

            <div v-if="restartKeys.length" class="in-banner in-banner--warn"><ShellIcon name="alert" :size="15" /><span>{{ $t('InstanceV2.restart_needed', { keys: restartKeys.join(', ') }) }}</span></div>
            ...
            <p class="ah-small">{{ $t('InstanceV2.settings_lead') }} <a :href="guide('configure')" target="_blank" rel="noopener">{{ $t('InstanceV2.guide') }}</a></p>
                ...
                <div v-for="row in visible" :key="row.key" class="in-field">
                    <div>
                        <label class="in-field__label" :for="`f-${row.key}`">
                            {{ label(row) }}
                            <span v-if="row.locked" class="ah-chip ah-chip--mono" :title="$t('InstanceV2.locked_help')">{{ $t('InstanceV2.locked') }}</span>
                            <span v-if="row.restart" class="ah-chip ah-chip--warn">{{ $t('InstanceV2.restart') }}</span>
                        </label>
                    ...
                    <div class="in-field__control">
                        ...
                        <label v-else-if="row.type === 'boolean'" class="in-toggle">
                            ...
                            <span>{{ draft[row.key] === 'true' ? $t('InstanceV2.on') : $t('InstanceV2.off') }}</span>
                        </label>
                        <template v-else-if="row.secret">
                            <input :id="`f-${row.key}`" v-model="draft[row.key]" type="password" autocomplete="new-password" class="ah-input" :disabled="row.locked" :placeholder="row.value.set ? $t('InstanceV2.secret_set') : $t('InstanceV2.secret_unset')" />
                            <label v-if="row.value.set && !row.locked" class="ah-small in-clear"><input v-model="clear[row.key]" type="checkbox" class="ah-check" /> {{ $t('InstanceV2.clear_secret') }}</label>
                        </template>
                        ...
                        <span v-if="errors[row.key]" class="ah-field__error">{{ $t(`InstanceV2.err_${errors[row.key]}`) }}</span>
                        <span v-else-if="row.source === 'default' && !row.secret" class="ah-small">{{ $t('InstanceV2.using_default') }}</span>
                    </div>
                </div>
            </section>
        </template>
...
const label = (row) => (te(`InstanceV2.f_${row.key}`) ? t(`InstanceV2.f_${row.key}`) : row.label);
...
async function save() {
    ...
    try {
        ...
        seed(data.settings);
        $toast.success(t("InstanceV2.saved"));
    } catch (e) {
...
async function test() {
    ...
    try {
        ...
        testResult.value = { ok: res?.data?.status === true, text: res?.data?.statusText || t("InstanceV2.test_ok") };
    } finally {
        busy.value = false;
    }
}
