    ...
    <div class="billing__client-wrap ah-scroll">
        <p class="billing__client-note">
            ...
            <button type="button" class="ah-btn ah-btn--secondary ah-btn--sm" @click="copyLink">
                ...
                {{ linkCopied ? $t('Billing.link_copied') : $t('Billing.copy_link') }}
            </button>
        </p>

        <p v-if="clientView.loading && !view" class="billing__state">{{ $t('Billing.loading') }}</p>
        <div v-else-if="clientView.error" class="ah-empty">{{ $t('Billing.load_failed') }}</div>
        ...
        <div v-else-if="view" class="billing__client-frame">
            <header class="billing__client-head">
                ...
                <span class="ah-mono billing__client-shared">{{ $t('Billing.shared_by', { name: (view.project.sharedBy || '').toUpperCase() }) }}</span>
                <span class="ah-toolbar__spacer"></span>
                <span class="ah-chip ah-chip--mono">{{ $t('Billing.client_badge') }}</span>
            </header>
            ...
            <div class="billing__client-grid">
                <div class="billing__client-col">
                    <section class="ah-card billing__client-card">
                        <div class="billing__client-card-head">
                            <span class="ah-h3">{{ $t('Billing.progress') }}</span>
                            <span class="ah-mono billing__client-count">
                                {{ $t('Billing.milestones_complete', { done: view.progress.complete, total: view.progress.total }) }}
                            </span>
                        </div>
                        <div v-if="!view.progress.milestones.length" class="ah-empty">{{ $t('Billing.no_milestones') }}</div>
                    ...
                    <section class="ah-card billing__client-card billing__waiting">
                        <div class="billing__client-card-head">
                            <span class="ah-h3">{{ $t('Billing.waiting_on_you') }}</span>
                            <span v-if="view.waitingOnYou.length" class="billing__badge">{{ view.waitingOnYou.length }}</span>
                        </div>
                        <div v-if="!view.waitingOnYou.length" class="ah-empty">{{ $t('Billing.nothing_waiting') }}</div>
                        <div v-for="item in view.waitingOnYou" :key="item.id" class="billing__waiting-row">
                            ...
                            <span class="ah-btn ah-btn--sm" :class="item.action === 'review' ? 'ah-btn--primary' : 'ah-btn--secondary'">
                                {{ item.action === 'review' ? $t('Billing.review') : $t('Billing.open_item') }}
                            </span>
                        </div>
                    </section>

                    <section class="ah-card billing__client-card">
                        <span class="ah-h3">{{ $t('Billing.recent_updates') }}</span>
                        <div v-if="!view.updates.length" class="ah-empty">{{ $t('Billing.no_updates') }}</div>
                ...
                <div class="billing__client-col">
                    <section class="ah-card billing__client-card">
                        <span class="ah-h3">{{ $t('Billing.client_invoices') }}</span>
                        <div v-if="!view.invoices.length" class="ah-empty">{{ $t('Billing.no_invoices') }}</div>
                        <div v-for="inv in view.invoices" :key="inv.number" class="billing__client-invoice">
                            ...
                            <div class="billing__client-invoice-body">
                                ...
                                <div class="ah-small" :class="{ 'billing__due-warn': inv.payable }">
                                    {{ inv.status === 'paid' ? $t('Billing.paid_on', { date: dayLabel(inv.issuedDate) }) : $t('Billing.due_net', { date: dayLabel(inv.dueDate) }) }}
                                </div>
                            </div>
                        ...
                        <span v-if="view.payNext" class="ah-btn ah-btn--primary ah-btn--block ah-btn--sm">
                            {{ $t('Billing.pay_invoice', { number: view.payNext }) }}
                        </span>
                    </section>

                    <section v-if="view.canMessage" class="ah-card billing__client-card">
                        <span class="ah-h3">{{ $t('Billing.ask_us') }}</span>
                        <form class="ah-field" @submit.prevent="send">
                            <textarea
                                v-model="message"
                                class="ah-input ah-textarea"
                                :class="{ 'ah-input--error': messageError }"
                                :placeholder="$t('Billing.message_placeholder')"
                                maxlength="2000"
                                rows="3"
                            ></textarea>
                            <span v-if="messageError" class="ah-field__error">{{ messageError }}</span>
                            <div class="billing__message-foot">
                                <span class="ah-small">{{ $t('Billing.message_hint') }}</span>
                                <span class="ah-toolbar__spacer"></span>
                                <button type="submit" class="ah-btn ah-btn--primary ah-btn--sm" :disabled="sending">{{ $t('Billing.send') }}</button>
                            </div>
                        </form>
                    </section>

                    <p class="ah-card billing__privacy">{{ $t('Billing.client_privacy') }}</p>
                </div>
            </div>
        </div>
    </div>
...
const stepMeta = (m) => {
    if (m.state === "done") return t("Billing.done_on", { date: (dayLabel(m.completedDate) || "").toUpperCase() });
    if (m.state === "in_progress") return t("Billing.percent_due", { percent: m.percent, date: (dayLabel(m.dueDate) || "").toUpperCase() });
    return t("Billing.due_on", { date: (dayLabel(m.dueDate) || "").toUpperCase() });
};

const waitingMeta = (item) => {
    const parts = [];
    if (item.milestone) parts.push(item.milestone);
    if (item.waitingSince) parts.push(t("Billing.waiting_since", { date: dayLabel(item.waitingSince) }));
    if (item.dueDate) parts.push(t("Billing.needed_by", { date: dayLabel(item.dueDate) }));
    return parts.join(" · ");
};
...
const send = async () => {
    if (!message.value.trim()) {
        messageError.value = t("Billing.message_empty");
        return;
    }
    messageError.value = "";
    sending.value = true;
    ...
    if (result.ok) {
        message.value = "";
        $toast.success(t("Billing.message_sent"));
    } else {
        messageError.value = result.message || t("Billing.save_failed");
    }
};
...
const copyLink = async () => {
    ...
    if (!url) {
        $toast.error(t("Billing.link_failed"));
        return;
    }
    try {
