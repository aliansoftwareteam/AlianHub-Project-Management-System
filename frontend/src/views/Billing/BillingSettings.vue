<template>
    <form class="ah-card billing__settings" @submit.prevent="submit">
        <div class="billing__settings-grid">
            <div class="ah-field">
                <label class="ah-field__label" for="bs-client">{{ $t('Billing.client_name') }}</label>
                <input id="bs-client" v-model="form.clientName" class="ah-input" maxlength="160" />
            </div>
            <div class="ah-field">
                <label class="ah-field__label" for="bs-vendor">{{ $t('Billing.vendor_name') }}</label>
                <input id="bs-vendor" v-model="form.vendorName" class="ah-input" maxlength="160" />
            </div>
            <div class="ah-field">
                <label class="ah-field__label" for="bs-tax-label">{{ $t('Billing.tax_label_field') }}</label>
                <input id="bs-tax-label" v-model="form.taxLabel" class="ah-input" maxlength="40" />
            </div>
            <div class="ah-field">
                <label class="ah-field__label" for="bs-tax">{{ $t('Billing.tax_rate_field') }}</label>
            ...
            <div class="ah-field">
                <label class="ah-field__label" for="bs-terms">{{ $t('Billing.payment_terms') }}</label>
                <input id="bs-terms" v-model="form.paymentTermsDays" class="ah-input" type="number" min="0" max="365" step="1" />
            </div>
            <div class="ah-field">
                <label class="ah-field__label" for="bs-rate">{{ $t('Billing.cost_rate_label') }}</label>
        ...
        <div class="billing__settings-foot">
            <label class="billing__rule">
                ...
                <span>{{ $t('Billing.message_hint') }}</span>
            </label>
            <span class="ah-toolbar__spacer"></span>
            <button type="button" class="ah-btn ah-btn--ghost ah-btn--sm" @click="$emit('close')">{{ $t('Billing.cancel') }}</button>
            <button type="submit" class="ah-btn ah-btn--primary ah-btn--sm" :disabled="saving">{{ $t('Billing.save') }}</button>
        </div>
    </form>
</template>
...
const submit = () => {
    ...
    if (!Number.isFinite(percent) || percent < 0 || percent > 100) {
        error.value = t("Billing.save_failed");
        return;
    }
    error.value = "";
    emit("save", {
