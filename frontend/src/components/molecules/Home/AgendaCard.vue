<template>
    <section class="hc-card hc-agenda" style="flex: 1">
        <div class="hc-card__head">
            <span class="hc-card__title">{{ $t('HomeV2.agenda') }}</span>
            <span class="hc-agenda__nav">
                <button type="button" :aria-label="$t('HomeV2.day')" @click="$emit('shift', -1)">‹</button>
                <button type="button" @click="$emit('shift', 0)">{{ dayLabel }}</button>
                <button type="button" :aria-label="$t('HomeV2.day')" @click="$emit('shift', 1)">›</button>
            </span>
        </div>
        <router-link v-if="!connected" class="hc-connect" :to="connectTo">
            <span class="hc-connect__ico"></span>{{ $t('HomeV2.connect_calendar') }}
        </router-link>
        <p v-if="!items.length" class="hc-hint" style="margin: 0">{{ firstRun ? $t('HomeV2.agenda_hint') : $t('HomeV2.agenda_empty') }}</p>
        <div v-for="item in items" :key="item.id" class="hc-agenda__item">
            ...
            <div class="hc-block" :class="`hc-block--${item.kind}`" :title="item.title">
                <template v-if="item.kind === 'reminder'">{{ $t('HomeV2.reminder_prefix', { title: item.title }) }}</template>
                <template v-else-if="item.kind === 'focus'">{{ $t('HomeV2.focus_prefix', { title: item.title || durationLabel(item) }) }}</template>
