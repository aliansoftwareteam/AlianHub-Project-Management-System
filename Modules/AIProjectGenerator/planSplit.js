...
const attachSplit = async (plan, { companyId, assumptions, agents, skills } = {}) => {
    const live = agents || await loadLiveAgents(companyId);
    const manifest = skills || (companyId ? await skillRecord.listSkills(companyId) : null);
    const labelled = labelPlan(plan || {}, live, manifest);
