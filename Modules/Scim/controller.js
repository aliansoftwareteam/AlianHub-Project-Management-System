...
const resourceOf = async (req, row) => {
    const gu = R.sharedRecordVisible(row) ? await prov.getGlobalUser(row.userId) : null;
    return R.toScimUser(gu, row, scimBaseOf(req));
};
...
exports.listUsers = async (req, res) => {
    try {
        ...
        const users = await prov.getGlobalUsersByIds(wanted.filter(R.sharedRecordVisible).map((m) => m.userId));
        const byId = {};
...
exports.getUser = async (req, res) => {
    try {
        const cu = await prov.getCompanyUser(req.scimCompanyId, req.params.id);
        if (!cu) return res.status(404).json(R.scimError(404, 'User not found.'));
        return res.json(await resourceOf(req, cu));
...
exports.createUser = async (req, res) => {
    try {
        ...
        const input = {
            ...
            defaultRoleType: (req.scimConfig && req.scimConfig.defaultRoleType) || 3,
        };
        if (!(await prov.mayProvisionDirectly(companyId, email, existing))) {
            const row = await prov.invite(companyId, input);
            audit(req, { action: 'scim.user_invite', entityType: 'member', entityId: String(row._id), entityName: email });
            return res.status(201).json(await resourceOf(req, row));
        }
        const { uid } = await prov.provision(companyId, input);
        audit(req, { action: 'scim.user_provision', entityType: 'member', entityId: String(uid), entityName: email });
        return res.status(201).json(await resourceOf(req, await prov.getCompanyUser(companyId, uid)));
...
exports.replaceUser = async (req, res) => {
    try {
        ...
        const cu = await prov.getCompanyUser(companyId, id);
        if (!cu) return res.status(404).json(R.scimError(404, 'User not found.'));
        await prov.updateName(companyId, id, {
            givenName: req.body.name && req.body.name.givenName,
            familyName: req.body.name && req.body.name.familyName,
        });
        if (req.body.active !== undefined) await prov.setActive(companyId, id, req.body.active !== false);
        return res.json(await resourceOf(req, await prov.getCompanyUser(companyId, id)));
...
exports.patchUser = async (req, res) => {
    try {
        ...
        if (ops.givenName !== undefined || ops.familyName !== undefined) await prov.updateName(companyId, id, ops);
        if (ops.active !== undefined) {
            await prov.setActive(companyId, id, ops.active);
            audit(req, { action: ops.active ? 'scim.user_activate' : 'scim.user_deactivate', entityType: 'member', entityId: String(id) });
        }
        return res.json(await resourceOf(req, await prov.getCompanyUser(companyId, id)));
