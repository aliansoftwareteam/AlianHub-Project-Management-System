...
| `GET /step-types` | the manifest: every contract's config and output fields, with their types, labels and what each type requires, plus the fan-out and loop bounds |
| `GET /definitions` | the company's saved workflows, most recently edited first |
| `POST /definitions` | saves one. Always turned off, whatever the body says |
| `PUT /definitions/:id` | replaces one's name, steps and bounds; `enabled` is not among them |
...
back first. `enabled` is therefore never taken from a create or an update body —
...
would be granted of the run's deadline and budget by `hop.allow`, which steps
