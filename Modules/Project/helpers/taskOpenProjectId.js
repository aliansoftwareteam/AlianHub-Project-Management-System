...
function taskMatchesBoard(row, { sprintId, projectId } = {}) {
...
function bindSprintTaskSource({ searched, searchRows, storedRows, sprintId, projectId } = {}) {
...
function rowHasPaintedLabel(row) {
    if (!row || row.deletedStatusKey || !firstId(row._id, row.id)) return false;
...
function countBoundPaintedRows(groups, storeTasks) {
