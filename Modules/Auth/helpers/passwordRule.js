...
const PASSWORD_PATTERN = new RegExp(`^(?=.*[A-Z])(?=.*[a-z])(?=.*[0-9])(?=.*[^A-Za-z0-9\\s]).{${MIN_PASSWORD_LENGTH},}$`);

const PASSWORD_RULE_MESSAGE = `Use at least ${MIN_PASSWORD_LENGTH} characters, including an uppercase letter, a lowercase letter, a number and a symbol.`;

const meetsPasswordRule = (password) => typeof password === 'string' && PASSWORD_PATTERN.test(password);

module.exports = { MIN_PASSWORD_LENGTH, PASSWORD_PATTERN, PASSWORD_RULE_MESSAGE, meetsPasswordRule };
