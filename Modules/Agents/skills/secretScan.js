...
const PATTERNS = Object.freeze([
    ...
    ['a GitLab token', /\bglp(?:at|tt)-[A-Za-z0-9_-]{20,}/],
    ['a HuggingFace token', /\bhf_[A-Za-z0-9]{30,}/],
    ['an npm token', /\bnpm_[A-Za-z0-9]{36,}/],
    ['a Slack app token', /\bxapp-[A-Za-z0-9-]{20,}/],
    ['a Slack webhook', /hooks\.slack\.com\/services\/T[A-Z0-9]+\/B[A-Z0-9]+\/[A-Za-z0-9]{16,}/],
    ['a SendGrid key', /\bSG\.[A-Za-z0-9_-]{16,}\.[A-Za-z0-9_-]{16,}/],
    ['a Google OAuth token', /\bya29\.[A-Za-z0-9_-]{20,}/],
    ['a basic auth credential', /\bBasic\s+(?=[A-Za-z0-9+/]*\d)(?:[A-Za-z0-9+/]{4}){3,}(?:[A-Za-z0-9+/]{2}==|[A-Za-z0-9+/]{3}=)?(?![A-Za-z0-9+/=])/],
    ['a presigned link', /X-Amz-Signature=[0-9a-f]{64}/i],
    ...
    ['a key in a link', /(?:^|[^A-Za-z0-9_])(?:api[_-]?key|apikey|access[_-]?token|auth[_-]?token|token|secret|client[_-]?secret|password|passwd|pwd)=(?=[^&#\s{]*\d)(?=[^&#\s{]*[A-Za-z])[^&#\s{]{12,}/i],
]);
