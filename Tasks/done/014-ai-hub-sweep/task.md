...
Status: done 2026-09-12 · every defect fixed and merged 2026-09-05 (PR #543, `1fd5d178`, build 20) · closed on the task 028 precedent (owner decision 2026-09-12): the remaining member sweep is tracked as an open owner check in HANDOFF.md, not as an open task. Carried forward: the member-role **browser** sweep of the `/ai` screens (the API half was covered by task 034 and its findings AGT-01 to AGT-11 fixed in PR #620; no `/ai` screen has ever been opened as a member) and the unchecked `agent_run` notification row, which still sends the agent id as the sender (`Modules/Agents/runs.js:340`)
