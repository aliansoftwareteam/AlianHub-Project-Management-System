...
- [x] Step 4: Pre-flight token estimate, reservation against the tenant budget, reconciliation after; the routing decision a — #668 `7331c646`
...
- [x] Exit gate met and gates green — #670 `6fa88bc4` and the owner and member sweeps at build 147
...
| 2026-09-12 | Routing policy moved into workspace settings (#672 `fefab887`) and swept at build 147: the owner sees the four task classes with input budgets, quality floor, latency target and a preferred model from the priced list, with the flag banner; a member gets no nav entry and the URL lands on My Profile. All four interface rows are now swept by the owner and by a member, and the exit gate passed in #670. |

## Last step
Complete: four steps merged, the blackholed-provider gate passed, every interface row swept. Moved to done.
