...
### 2026-09-10 — closed, superseded by 008
- The Pages half landed on beta independently: `Modules/Pages/routes.js` (`POST /api/v2/pages/ai`)
  and `router/pages/index.js`. The kiln shell was retired by 008, whose handoff tokens supersede it.
- Left behind: the `--kiln-*` aliases in `tokens.css:384-389` and the legacy Header kiln classes
  behind the `ah.legacyNav` toggle — tracked in task 021.
