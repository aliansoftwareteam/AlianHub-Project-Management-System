...
### 2026-09-10 — closed, folded into 013
- 013's task.md retires the duplicate id 005 by folding first-run into it. Twelve of the thirteen
  gaps verified on beta (64f4f507).
- G10 ("every permission shows a plain-language description") regressed when 100ee0ea rewrote
  SecurityPermissions onto `molecules/Setting/PermissionMatrix.vue`, which renders only the empty
  seeded `rule.desc`; the 98 `PermissionDesc.*` sentences at `en.js` ~4762 have no consumer. That
  item now sits on 013's remaining list.
