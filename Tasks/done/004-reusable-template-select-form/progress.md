...
### 2026-09-10 — closed
- Done. fc4bc395, df7301a5 and 747833aa (PR #487). `TemplateSelectForm.vue` is consumed by the
  three forms mounted from `ProjectSettingSidebar.vue` and `CreateTemplate.vue`.
- The create-project wizard context is moot since the redesign (100ee0ea) replaced
  CreateProjectSidebar with a template picker. Leftovers — the stale `.wizard-step-fill` comment at
  `TemplateSelectForm.vue:189` and the inert `.tsf-fill-list` classes — are tracked in task 021.
