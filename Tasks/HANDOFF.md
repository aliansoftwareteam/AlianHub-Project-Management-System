...
## State of `beta` (df812206, `14.36.0-beta.383`)

- **Overnight 2026-09-23/24: builds 303–383 (#834–#914)**, listed in `docs/BETA-LOG.md`.
...
- **Upgrade steps for a deployed instance:**
  ...
  7. Operator routes from #655 (`x-preset-key`, `POST /api/v1/setPresetCompany`), `TRACKER_PKCE_LEGACY_UNTIL` (#653), and the off-by-default flags listed in `docs/ENV.md` are unchanged.

## Next up

1. **In flight at session end:** the project-level group of row 111 (history text built on the server); row 115 (the remaining writes that name people); #916 (follow-up to #913).
2. **Owner questions:** row 113 (`--ink-3` contrast); review existing Google sign-in links (details in the owner's private notes); row 119 (decision 8 re-check once real downloads are logged).
3. **Owner actions:** edit the held-out question set (decision 12; Sprint 9 waits, decision 13); the sweeps in rows 77, 83 and 94 (a headless pass is done, #903); the Sprint 5 workflow screens (task 028); Stats, Upgrade and the Docker label (task 033); the quota recompute; the duplicate migration 021; rotating the two API keys named in the owner's local notes.
4. **Open follow-ups:** 14, 57, 77, 79, 80, 83, 84, 89, 94, 96, 98, 101, 111–119 in `Tasks/active/034-end-to-end-qa-programme/followups.md`.
