...
Updated 2026-09-18. Read this first, then `Tasks/index.md`. Overwrite this file at the end of every session.

## State of `beta` (94f6d206, `14.36.0-beta.225`)
...
- **Sprints 7 and 8 run in parallel** (tasks 030 and 031). Each `task.md` holds the slice plan in merge order, the ownership split between the two sprints, and every owner decision; each `progress.md` ticks the merged slices with their build numbers.
  - **Sprint 7 merged:** slices 0 (#735, build 210), 1 (#739, 215), 9 (#742, 217), 5 (#746, 220), 2 (#745, 221) and 3 (#747, 223). Ask applies the private-sprint rule; the retrieval interface, the chunk store and pages, comments, transcripts and workspace pages sit behind `KNOWLEDGE_RETRIEVAL` and `KNOWLEDGE_INDEXER`, both off by default; `performance.read` sits behind `AGENT_PERFORMANCE_READ`, off.
  - **Sprint 8 merged:** slices 0b (#737, 212), 0a (#738, 213), 1 (#740, 214), 2 (#743, 218), 7 (#744, 219), 3 (#749, 222), 3b (#751, 224) and 5 (#750, 225). Browser sessions pass through the permission check in off, report and enforce modes (`PERMISSION_ENFORCEMENT_MODE`, off, set per workspace); every task write maps to its key, writes only in the authorised company and only its own fields; tokens have mandatory expiry and explicit scopes behind `API_TOKEN_STRICT`, off; the audit log is a keyed hash chain behind `AUDIT_CHAIN`, off.
  - **In flight on 2026-09-18:** Sprint 7 slice 6 (embeddings and hybrid retrieval); Sprint 8 slices 3c (task index validation, actor and counts from stored data), 4 (enforcement console) and 7b (workspace token-expiry list).
  - **Next:** Sprint 7 slices 4, 8, 10, 7, 11; Sprint 8 slices 6, 8, 9, 10, 11, 12.
- **Every Sprint 7 decision is settled** (see task 030 Decisions). Sprint 8 still needs: a maximum token lifetime if the architecture document names none (slice 7), the secrets key (9), and in-app egress or a proxy container plus what an empty allowlist means (10).
- **Owner's local server:** build 225, main checkout at `94f6d206`, frontend rebuilt 2026-09-18 (the audit screen's integrity states and the strict-mode tokens screen are in it). Migrations 024–026 and 028–030 are applied there.
- **Upgrade steps for a deployed instance:**
  ...
  2. Migrations run at server start: 013 (SSO and SCIM seats), 024–026 (knowledge indexes), 028 (clears a stored field on tasks), 029 (permission decisions), 030 (audit chain indexes).
  ...
  5. Flags that are off by default, where off is today's behaviour exactly: `WORKFLOW_ENGINE`, `KNOWLEDGE_RETRIEVAL`, `KNOWLEDGE_INDEXER`, `AGENT_PERFORMANCE_READ`, `PERMISSION_ENFORCEMENT_MODE`, `API_TOKEN_STRICT`, `AUDIT_CHAIN` (needs `AUDIT_CHAIN_KEY`, at least 32 characters; changing the key later is not supported).
...
1. **Merge the four in-flight slices**, each after an independent review; every slice this week that changed who may see or do what had real defects found by review after green CI (see "Things learned").
2. **Owner decisions still open:** Sprint 8 slices 7, 9 and 10 (above); the older follow-ups 32, 35, 37 and 56.
...
4. **Remaining follow-ups:** 14, 26, 55–64 and 65–77 in `Tasks/active/034-end-to-end-qa-programme/followups.md`. Item 72 (an integration-suite order dependency) will bite any full local run that happens to order `invited-owner` before `instance-fixes`.
5. **Owner checks still open:** the owner and member sweeps listed in follow-up 77 (Ask's panel, the tokens screen under strict, the audit log's integrity states); the Sprint 5 workflow screens (task 028); Stats and Upgrade and the Docker label (task 033); the `/ai` and trust-layer member sweeps (014, 016); the day-to-day screens at 1280 and 800 px (013); every new interface row as it lands.
...
- **Merge churn:** PRs that add i18n keys conflict on `*.pending.json` after each merge; keep every key from both sides. `docs/ENV.md` conflicts are resolved by taking beta's copy and re-running `node scripts/env-doc.js`. Keep git's default merge subject.
