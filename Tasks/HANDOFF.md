...
- **Sprints 0 to 6 are closed.** 0 to 2 by #676 (build 150), 3 and 4 by #673 (148), 5 by #684 (159), and 6 by #713 (187) and #714 (189) — the Skill Library is a real library, the three duplicated input tables are gone, and `digest.ceo` and `project.guide` are documents with their code files deleted. **Sprint 7 (task 030) and Sprint 8 (task 031) are next and the roadmap allows them in parallel.**
...
4. **Owner actions — an agent cannot do these:**
   - **Sprint 6's two open acceptance bullets:** an admin creating a skill, assigning it and running it on a task end to end (needs `AI_API_KEY`), and owner + member browser sweeps of the Skill Library, the editor, the dry-run panel and agent settings → skills.
...
5. **Sprint 7 (task 030, knowledge and retrieval) and Sprint 8 (task 031, security hardening) are next.** Both depend only on 024, which is closed, and the roadmap says they can run on parallel tracks. Sprint 6 closed with two acceptance bullets open, both owner actions: the end-to-end run needs a live model key, and the four new screens need an owner and member browser sweep.
