...
- **The project-screen UI programme is complete.** An 85-finding audit of the project screens became eleven batch PRs, all merged: #695 (build 170) repaired the list grid, the sticky table head and dark-mode contrast; #696 (171) added the four missing token tiers and moved `.ah-label` onto `var(--ink-label)` across 78 files; #697 (172) stopped the task empty states blaming a filter nobody set; #698 (173) made the task-filter chip keyboard-reachable; #699 (174) fixed due-date grouping, whose Mongo bounds were raw second counts read back as `new Date(seconds)`, so Overdue matched nothing and Next matched everything; #700 (175) showed every sprint in the table by moving the IntersectionObserver sentinel out of the `v-if` that hid empty groups; #701 (176) cut the toolbar to one row; #702 (177) gave board cards real due dates and stopped the unconditional HUMAN badge and the empty pill; #703 (178) gave list rows the company's vocabulary; #704 (179) showed the project name once; #705 (182) rebuilt the views palette and gave the tabs a keyboard story. Alongside them: #694 (169) counted closed subtasks on collapsed parent rows, #706, #707, #708, #709 and #711 (180, 183, 184, 185, 186) took the views off literal colours — 354 down to 74, the rest deliberate and commented — and #710 (181) landed a PTO capacity assertion on a working day, after a floating `now + 40 days` crossed onto a Saturday and turned `beta` red. Five follow-ups the programme found but did not fix are items 59 to 63 in `Tasks/active/034-end-to-end-qa-programme/followups.md`.
