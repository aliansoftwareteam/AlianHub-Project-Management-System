...
Updated 2026-09-19. Read this first, then `Tasks/index.md`. Overwrite this file at the end of every session.

## State of `beta` (8d3a292e, `14.36.0-beta.232`)
...
- **Sprints 7 and 8 run in parallel** (tasks 030 and 031). Each `task.md` holds the slice plan in merge order, the ownership split between the two sprints, and every owner decision; each `progress.md` ticks the merged slices with their build numbers.
  ...
  - **Sprint 8 merged:** slices 0b (#737, 212), 0a (#738, 213), 1 (#740, 214), 2 (#743, 218), 7 (#744, 219), 3 (#749, 222), 3b (#751, 224), 5 (#750, 225), 7b (#753, 227), 4 (#754, 228), 3c (#756, 231) and 6 (#757, 232). Browser sessions pass through the permission check in off, report and enforce modes (`PERMISSION_ENFORCEMENT_MODE`, off, set per workspace); every task write maps to its key, writes only in the authorised company and only its own fields; tokens have mandatory expiry and explicit scopes behind `API_TOKEN_STRICT`, off; the audit log is a keyed hash chain behind `AUDIT_CHAIN`, off. The instance console has an Enforcement tab (mode per workspace, would-be denials, the 14-quiet-days mark); owners and admins see the tokens that still need an expiry; task index queries are validated and the actor and counts come from stored data; a run that read outside content is marked and its risky actions go to approval behind `AGENT_TAINT_ROUTING`, off.
  - **In flight on 2026-09-19:** Sprint 8 slices 8 (step credentials, #760), 9 (secrets store, #759, migration 031) and 10 (egress allowlist, #758), each in a review fix round; Sprint 8 slice 12 (content security policy, `CSP_MODE`); Sprint 7 slice 4 (file text and project guides, which also sets `origin` on every passage).
...
- **Owner's local server:** build 232, main checkout at `8d3a292e`, frontend rebuilt 2026-09-19 (the Enforcement tab, the token-expiry list and the run view's outside-content mark are in it). Migrations 024–030 are applied there.
- **Upgrade steps for a deployed instance:**
  ...
  2. Migrations run at server start: 013 (SSO and SCIM seats), 024–027 (knowledge indexes and embeddings), 028 (clears a stored field on tasks), 029 (permission decisions), 030 (audit chain indexes).
...
1. **Merge the five in-flight slices**, each after an independent review; every slice this week that changed who may see or do what had real defects found by review after green CI (see "Things learned").
