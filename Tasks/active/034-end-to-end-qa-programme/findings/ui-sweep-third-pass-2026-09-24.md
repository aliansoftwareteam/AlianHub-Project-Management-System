...
| U3-34 | Agents as teammates, People | owner | all / both | The guest row shows role "Member" and access "Everything"; every person's subtitle in People reads "Owner" (the fixture's designation 0), so the job title reads like a role | low | no — needs a look at the role label mapping |
...
| U3-38 | Doc view | both | all / both | A long title is clipped inside the single-line title field with no wrap or ellipsis; a stray dot sits left of the editor body | low | no |
