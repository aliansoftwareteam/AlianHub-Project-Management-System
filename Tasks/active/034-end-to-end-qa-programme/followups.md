...
| 103 | Delete the `prReview.js` code skill once `SKILL_EXTERNAL_READS` defaults on and the `pr.summary` data seed replaces it. **Closed** by this PR: the owner decided `SKILL_EXTERNAL_READS` alone switches `pr.summary` to its seed and the code version is deleted; with the flag off the skill is listed as unavailable with the reason. | #819 |
...
| 110 | `pr.summary` runs its code version unless **both** `SKILL_EXTERNAL_READS` and `PR_SUMMARY_AS_DATA` are on, and nothing on screen says which ran; only the replay's fetch row tells. Either retire the code version (see 103) or show the version on the run. **Closed** by this PR for the flag: `PR_SUMMARY_AS_DATA` is removed and the code version deleted, so only the seed can run. | 2026-09-23 acceptance run |
