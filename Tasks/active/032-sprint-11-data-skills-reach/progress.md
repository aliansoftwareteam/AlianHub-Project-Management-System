...
| 2026-09-23 | Owner decision: `SKILL_EXTERNAL_READS` alone switches `pr.summary` to its data version, and the code version is deleted (follow-up 103). `PR_SUMMARY_AS_DATA` is removed; `Modules/Agents/skills/prReview.js` and the `agentFetch.fetchPage` wrapper only it used are deleted. With the flag off, `pr.summary` (alias `risk.flags`) still resolves and lists, carries `unavailable` with the reason in the skill list, the agent manifest and the Skill Library, and a run fails with `external_reads_not_available` before anything is fetched. Seeds are pinned by a hash of the document as written, the same with the flag on or off. #874, branch `feat/pr-summary-data-only`. |
