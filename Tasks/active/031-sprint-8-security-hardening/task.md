...
- An API token lives at most 365 days (owner, 2026-09-21).
- The Google Drive picker loads in a separate popup page with its own policy, so the main app's enforced policy never allows eval; until it ships, installs that use Drive stay on `report` (owner, 2026-09-21).
- Stored-file downloads are scoped to records the caller may open (follow-up 84, #794). Enforce only if review finds no legitimate caller broken, else report for one release first (owner, 2026-09-21); the review found four, so it ships in `report` mode.
