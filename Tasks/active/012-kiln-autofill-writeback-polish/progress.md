...
- Bot 3 addendum: Comments tab copies the deep-link path (local `activeTab` only, no `router.replace`). Tab pointer dispatches `kiln-task-tab` so a queued Open-chip click cannot arm the title; title edit requires mousedown+click on the h4. Thread scroll stays inside `.msg__container` (`overscroll-behavior: contain`, `scrollIntoView` nearest). Autofill still has one copper Autofill/Fill empty above Summary. Placeholder `DD/MM/YYYY` is empty; Due seeds from sprint dates (`24 - 28 Aug 2026` → 2026-08-28) with Assignee/Owner/Due rows and per-row apply. Gantt not started.
