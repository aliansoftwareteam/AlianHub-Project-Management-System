...
- Bot 2 kiln lock: Comments tab only switches tabs — `router.replace({ query })` keeps params, sidebar backdrop does not close the panel, title edit is a direct title click. Escape is overflow …, then title, then Autofill/strip, then panel. Autofill copper Autofill/Fill empty stays in the main column above Summary; the suggestion card can dismiss without killing Fill empty. Assignee apply writes the rail chip only when that row is checked; Owner apply writes the custom field. No shared OWNER tag. Gantt not started.
